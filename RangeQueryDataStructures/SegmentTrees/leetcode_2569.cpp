 ...
 */
template <typename T, typename U>
class flipUpdate : public UpdateStrategy<T, U>
{
    public:
        T apply(const T &value, const U &updateVal, int low, int high) override
        {
            if(updateVal)
                return (high -low + 1) - value; // Assuming value is count of 1s in the range, flip it to count of 0s
            else
                return value; // No change if updateVal is false (0)
        }

        U combine(const U &oldLazy, const U &newLazy) override
        {
            return oldLazy ^ newLazy; // XOR for flip operation
        }
...
class segTree
{
    ...
    public:
         ...
         */
        segTree(vector<T> &num1, vector<T> &num2, unique_ptr<MergeStrategy<T>> mergeStrat, unique_ptr<UpdateStrategy<T, U>> updateStrat);
        ...
        void setSumNum2(long long sum)
        {
            sumNum2 = sum;
        }

        long long getSumNum2() const
        {
            return sumNum2;
        }
};
...
template <typename T, typename U>
segTree<T, U>::segTree(vector<T> &num1, vector<T> &num2, unique_ptr<MergeStrategy<T>> mergeStrat, unique_ptr<UpdateStrategy<T, U>> updateStrat):
         mergeStrat_(move(mergeStrat)),
         updateStrat_(move(updateStrat)),
         numElem(num1.size())
{
    tree_.assign(4 * numElem, segTreeNode<T, U>());
    sumNum2 = accumulate(num2.begin(), num2.end(), 0LL);

     // Now build the tree

    // Now build the tree 
    build(num1, 0, numElem -1, 0);   
}
...
public:
    vector<long long> handleQuery(vector<int>& nums1, vector<int>& nums2, vector<vector<int>>& queries) {
        segTree<int, bool> mySegTree(nums1, nums2, make_unique<SumMain<int>>(), make_unique<flipUpdate<int, bool>>());
        ...
        for(const auto &item : queries)
        {
            if(item[0] == 1)
            {
                mySegTree.update(item[1], item[2], true);
            }
            else if(item[0] == 2)
            {
                mySegTree.setSumNum2(mySegTree.getSumNum2() + 
                (long long)mySegTree.search(0, nums1.size() - 1) * (long long)item[1]);
            }
            else
            {
                results.push_back(mySegTree.getSumNum2());
            }
        }
        return results;
    }
