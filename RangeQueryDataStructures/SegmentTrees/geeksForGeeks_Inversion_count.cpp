...
void fenwickTree<T, U>::buildTree(const vector<T> &inArr)
{
    ...
    {
        update(i, static_cast<U>(inArr[i]));
    }
}
...
class Solution {
  public:
    int inversionCount(vector<int> &arr) {
        ...
        vector<int> freqArr(10000, 0);

        for (int i = 0; i < arr.size(); i++) {
            freqArr[arr[i]-1]++;
        }

        fenwickTree<int, int> tree(freqArr, make_unique<sumMergeStrategy<int>>(), make_unique<sumUpdateStrategy<int, int>>(), 
        ...
        for (int i = 0; i < arr.size(); i++) {
            ...
            // Get the count of elements greater than the current element
            if (arr[i] < 10001) { // Ensure we don't go out of bounds
                inversionCount += tree.rangeQuery(1, arr[i] -1);
            }
        }
